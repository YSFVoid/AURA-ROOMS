export function paginate(items, page, pageSize) {
    ...
    return { items: items.slice(start, end), page: safePage, totalPages };
}

export function parseFooterPage(footerText) {
    if (!footerText) return { page: 1, totalPages: 1 };

    const match = footerText.match(/Page\s+(\d+)\/(\d+)/i);
    if (!match) return { page: 1, totalPages: 1 };
    ...
    return { page: Math.max(1, page), totalPages: Math.max(1, totalPages) };
}

export function setPageFooter(embed, page, totalPages) {
